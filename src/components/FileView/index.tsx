...
function FileView({ initialFileTree }: { initialFileTree: TreeType }) {
        ...
        <div className="toolbar-view">
          {isFolder && (
            <>
              <FiFolderPlus
                title="New Folder"
                onClick={() => addItemInTree("folder", node)}
              />
              <FiFilePlus
                title="New File"
                onClick={() => addItemInTree("file", node)}
              />
            </>
          )}
          <FiEdit
          ...
          {node.id === "root" && (
            <VscCollapseAll onClick={handleCollaspseAll} />
          )}
        </div>
      </div>
    );
  ...
  const deleteFromTree = (o: TreeType, id: string | undefined) => {
    function getNode(a: { id: string; children: any[] }, i: number) {
      if (a.id === id) {
        index = i;
        return true;
      }
